...
export class CameraControls extends EventDispatcher {
	...
	fitTo( box3OrObject: _THREE.Box3 | _THREE.Object3D, enableTransition: boolean, options: FitToOption = FIT_TO_OPTION_DEFAULT ): void {
		...
		const side          = options.side          || SIDE.FRONT;
		...
		const boundingBox = ( box3OrObject as any ).isBox3
		...
		let boundingWidth  = size.x;
		let boundingHeight = size.y;
		let boundingDepth  = size.z;

		const boundingBoxCenter = boundingBox.getCenter( _v3A );
		let cx = boundingBoxCenter.x + ( paddingRight - paddingLeft   ) * 0.5;
		let cy = boundingBoxCenter.y + ( paddingTop   - paddingBottom ) * 0.5;
		let cz = boundingBoxCenter.z;
		...
		switch ( side ) {
			...
			case SIDE.UP:
			case SIDE.DOWN:
				[ boundingHeight, boundingDepth ] = [ boundingDepth, boundingHeight ];
				[ cy, cz ] = [ - cz, - cy ];
				break;

			case SIDE.RIGHT:
			case SIDE.LEFT:
				[ boundingWidth, boundingDepth ] = [ boundingDepth, boundingWidth ];
				[ cx, cz ] = [ cz, - cx ];
				break;
		...
		const distance = this.getDistanceToFit(
			boundingWidth  + paddingRight + paddingLeft,
